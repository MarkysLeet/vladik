  ...
  return (
    <div className="absolute inset-0 -z-10 bg-paper-white overflow-hidden">
      <Canvas camera={{ position: [0, 0, 5], fov: 45 }}>
        ...
        <spotLight position={[5, 10, 5]} intensity={1} penumbra={1} color="#ccff00" />
        <pointLight position={[-5, -5, -5]} intensity={0.5} color="#0A2F1F" />

        {/* Floating Paper Sheets */}
        <PaperSheet position={[-1.5, 0.5, 0]} rotation={[0, 0.2, 0.1]} scale={[2, 2.8, 1]} color="#ffffff" />
        <PaperSheet position={[1.8, -0.5, -1]} rotation={[0, -0.2, -0.1]} scale={[1.5, 2.1, 1]} color="#f0f0f0" />
        <PaperSheet position={[0, -2, -2]} rotation={[0.5, 0, 0]} scale={[3, 4.2, 1]} color="#ffffff" />
      </Canvas>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center max-w-4xl px-4">
          ...
          <p className="text-xl text-forest-green/80 max-w-2xl mx-auto">
            Industrial precision with messenger ease. No carts, no forms.
          </p>
        </div>
      </div>
    </div>
